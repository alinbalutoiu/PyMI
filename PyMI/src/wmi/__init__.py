...
class _Instance(object):
    ...
    def __getattr__(self, name):
        try:
            ...
            # The old WMI translates automatically into WMI objects those class
            # properties that are references. To maintain the compatibility
            # with the old WMI, those class properties that are references are
    ...
    if isinstance(value, mi.Instance):
        if el_type == mi.MI_INSTANCE:
            ...
            if convert_references:
                # The object will be returned only in the case in which the
                # reference is the property of a class, otherwise the
                # path is returned
